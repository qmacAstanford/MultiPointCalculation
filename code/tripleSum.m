...
ratio=abs( max(max(abs(kron(vL',vR').*M1)))*prefac/o_a_b );
if isnan(ratio)
    o_a_b_ac=100;
else
...
ratio=abs(max(max(abs(kron(vL',vR').*M1)))*prefac...
      /abs(o_ab));
if isnan(ratio)
        o_ab_ac=100;
else
...
ratio=abs(max(max(abs(kron(vL',vR').*M1)))*prefac/oa_b);
if isnan(ratio)
    oa_b_ac=100;
else
...
% if abs((brute-bestAns)/brute)>0.05
%     sprintf('a=%g, b=%g, best=%d, bestAns=%g, brute=%g',a,b,best,bestAns,brute)
