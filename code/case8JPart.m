...
if N<4
    ...
    ndigits=14.5; %accuarcy of floating point numbers (all little conservative)
        ...
        if max(abs([N*(1-a), N*(b-c)/b]))< goodEnough && a-b>0.1
            out=oa_bc;
            return
        end        
        ...
        if isnan(ratio)
            ...
            oa_bc_ac=max(abs([N*(1-a), N*(b-c)/b, ratio*10^(-ndigits)]));
        end
        ...
        if isnan(ratio)
            ...
            o_ab_c_ac=max(abs([N*(a-b)/b, ratio*10^(-ndigits)]));
        end
        ...
        if isnan(ratio)
            ...
            o_a_bc_ac=max(abs([N*(a-b)/b, ratio*10^(-ndigits)])); 
        end
        ...
        if isnan(ratio)
            ...
            o_abc_ac=max(abs([N*(a-b)/a, N*(b-c)/a, ratio*10^(-ndigits)]));
        end
        ...
        answs=[oabc,...
               ...
               abz_c,...
               a_b_c];
%         if 0.1<abs((answs(best)-brute)/brute)
