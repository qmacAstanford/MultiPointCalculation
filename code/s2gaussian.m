function s2=s2gaussian(N,NM,LAM,FA,k,d)
...
% valne1=2*(-1).*R.^(-2).*(1+N.*((-1)+Z0)+(-1).*Z0.^N);
% valne2=2*(-1).*R.^(-2).*((-1)+Z0).^2.*Z0.^(-1).*((-1)+Z1).^(-2).*Z1.*(1+N.* ...
%                 ((-1)+Z1)+(-1).*Z1.^N);

% s2(1,1)=s2(1,1)+(2*FA*N*valeq+FA^2*valne1+FA*(1-FA)*valne2);
% s2(1,2)=s2(1,2)+(FA*(1-FA)*valne1-FA*(1-FA)*valne2);
% s2(2,1)=s2(1,2);
% s2(2,2)=s2(2,2)+((1-FA)*valeq+(1-FA)^2*valne1+FA*(1-FA)*valne2);
