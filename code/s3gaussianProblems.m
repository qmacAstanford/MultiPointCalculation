...
%             valeq=R1.^(-3).*(2+NM.*R1+((-2)+NM.*R1).*Z1);
            valeq=R1.^(-3).*(-2*expl(3,NM*R1)+NM*R1*expl(2,NM*R1));
        end
    else
%         valeq=R1.^(-2).*(R1+(-1).*R2).^(-1).*R2.^(-2).*((-1).*NM.*R1.*R2.^2+ ...
%             R2.^2.*((-1)+Z1)+R1.^2.*(1+NM.*R2+(-1).*Z2));
         valeq=(R2-R1).^(-1).*(R2.^(-2).*expl(2,NM*R2)-R1.^(-2).*expl(2,NM*R1));
    end
%     valeq=finite(valeq);
...
%             valeq=R1.^(-3).*((-1)+Z1).*Z1.^(-1).*(1+((-1)+NM.*R1).*Z1);
            valeq=R1.^(-3).*(-expl(4,NM.*R1)-expl(4,-NM.*R1)+NM*R1.*expl(2,NM*R1));
        end
    else
%         valeq=R1.^(-2).*(R1+(-1).*R2).^(-1).*R2.^(-1).*((-1)+Z1).*Z1.^(-1).*(R1+ ...
%             R2.*((-1)+Z1)+(-1).*R1.*Z2);
        valeq=(R2.^2.*R1.*(R1-R2))^(-1).*...
            (-1).*( R1.*expl(2,NM*R2)+...
                    R1.*expl(2,-NM*R2)+...
                    R2.*expl(1,-NM*R2).*expl(1,NM*R1) );
    end
...
%             -1)+Z2)).*((-1)+Z2).*Z2.^(-1);
         valeq=(R1.^2.*R2.*(R2-R1))^(-1).*...
            (-1).*( R2.*expl(2,NM*R1)+...
                    R2.*expl(2,-NM*R1)+...
                    R1.*expl(1,-NM*R1).*expl(1,NM*R2) );
    end
...
% Case 4: J1<J2<J3
    ...
    % on same monomer
    if abs((R1-R2)/(R1+R2))<MIN
        if abs(R1)<MIN
            valeq=power(NM,3)
            error('Fatal Warning, using NM^3')
        else
%             valeq=2.*NM.*R1.^(-2).*((-1)+(1/2).*(Z1.^(-1)+Z1));
            valeq=R1^(-2).*NM.*(expl(2,NM*R1)+expl(2,-NM*R1));
            error('Fatal warning, using R1=R2')
        end
    else
...
%     MIN=abs(exp(NM)*exp(R1)*(1-exp(MIN)));
    ...
    for I1=1:2
        for I2=1:2
            ...
            elseif abs(ZT1-ZT2)<MIN
                ...
                if (ZT1>0.2 && ZT2>0.2)
                    valne(I1,I2)=s3case4JnearOne(N,log(ZT1),log(ZT2));
                else
    ...
    if isnan(valeq)
        error('Fatal Warning, valeq is nan')
    end
    if abs(valeq)>1e10
        error('Fatal Warning, valeq is big')
    end
    if abs(valne)>1e10
        error('Fatal Warning, valne is big')
    end     
    valeq=finite(valeq);
    valne=finite(valne);
